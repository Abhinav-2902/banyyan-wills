...
export default async function EditorPage({ params }: EditorPageProps) {
  ...
  const isFinalized = will.status === "PAID" || will.status === "COMPLETED";
  let isPremiumEdit = false;

  if (isFinalized) {
    // Check if user is PREMIUM or has DEV BYPASS
    ...
    if (user?.subscriptionTier === "PREMIUM" || isDevBypass) {
      ...
      // Render completed will view for FREE users
      return (
        <div className="max-w-3xl mx-auto py-16 px-4 sm:px-6 lg:px-8 text-center">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
            ...
            <div className="mt-8 pt-8 border-t border-gray-100 text-sm text-gray-500">
               <p>Free users cannot edit completed wills. Upgrade to Premium for lifetime edits and version history.</p>
            </div>
          </div>
        </div>
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <MultiStepWillForm 
        initialData={willData ?? undefined} 
        willId={will.id} 
        isPremiumEdit={isPremiumEdit}
      />
    </div>
  );
}
